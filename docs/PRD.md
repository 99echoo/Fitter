...
### 1.5 현재 상태 및 전제
- UI는 아직 미구현 상태이며, 데이터셋 확정 전이므로 스키마/API 변경 가능성이 있습니다.
...
- 현재 모델 ID: `gemini-2.0-flash-exp-image-generation` (데이터셋/정책에 따라 변경 가능)
...
2. 피팅 서비스 페이지
   ├─→ Step 1: 얼굴 사진 업로드 (또는 샘플 선택)
   ├─→ Step 2: 전신 사진 업로드 (또는 샘플 선택)
   └─→ Step 3: 의상 선택
